...
for _k, _v in _REQUIRED.items():
    if not _v:
        print(f"  WARNING: secret '{_k}' is not set")
...
GLOBAL_EXCLUSIONS = [
    ...
    "publication of advertisement", "newspaper clipping", "advertisement in newspaper",
    "published in newspaper", "notice published in", "extract of newspaper",
    "newspaper cutting", "corrigendum", "erratum",
    "loss of share certificate", "duplicate share certificate",
    "sub-division of shares", "consolidation of shares", "transmission of shares",
    "intimation of record date", "change in registrar", "appointment of registrar",
]
...
CATEGORY_EXCLUSIONS = {
    "acqmerger": [
        ...
        "newspaper", "advertisement", "dividend", "record date", "book closure",
        "agm", "annual general meeting", "egm", "extraordinary general meeting",
        "postal ballot", "voting result", "rights issue", "public issue",
        "ipo", "fpo", "ncd", "non-convertible debenture", "commercial paper",
    ],
    "demerger": [
...
RULES = {
    "results": {
        ...
        "sheet": SHEET_RESULTS, "channel": None,
        "emoji": "📊", "label": "Financial Results", "priority": 4,
    },
    "investors": {
        "keywords": [
        ...
        "sheet": SHEET_INVESTORS, "channel": None,
        "emoji": "📞", "label": "Investors Meet / Concall", "priority": 3,
    },
    "acqmerger": {
        "keywords": [
            ...
            "business acquisition", "strategic acquisition", "strategic investment",
            "share purchase agreement", "binding term sheet", "definitive agreement",
            "letter of intent", "due diligence", "delisting", "substantial acquisition",
            "change in control", "promoter acquisition", "creeping acquisition",
        ],
        "sheet": SHEET_ACQMERGER, "channel": None,
        "emoji": "🤝", "label": "Acquisition / Merger", "priority": 2,
    },
    "demerger": {
        "keywords": [
            "demerger", "demerge", "demerged", "demerging", "spin-off", "spinoff",
            "spin off", "hive off", "hive-off", "hiving off", "carve-out", "carve out",
            "carved out", "scheme of demerger", "demerger ratio", "demerger consideration",
            "composite scheme of demerger",
        ],
        "sheet": SHEET_DEMERGER, "channel": None,
        "emoji": "🔀", "label": "Demerger", "priority": 1,
    },
    "mgmt": {
        "keywords": [
            "change in directorate", "change in director", "change in management",
            "appointment of director", "resignation of director", "cessation of director",
            "re-appointment of director", "reappointment of director",
            "appointment of managing director", "appointment of md",
            "appointment of ceo", "resignation of ceo", "appointment of cfo",
            "resignation of cfo", "appointment of coo", "appointment of cs",
            "appointment of company secretary", "resignation of company secretary",
            "change in key managerial", "kmp", "whole time director",
            "executive director", "independent director", "woman director",
            "additional director", "director retirement", "change in chairman",
            "appointment of chairman", "change in board", "board reconstitution",
        ...
        "sheet": SHEET_MGMT, "channel": None,
        "emoji": "👔", "label": "Change in Management", "priority": 5,
    },
}
...
def classify(title, body):
    ...
    for cat, rule in RULES.items():
        ...
        if any(ex in text for ex in excl):
            if cat == "acqmerger":
                strong = [
                    ...
                    "substantial acquisition", "change in control", "promoter acquisition",
                    "creeping acquisition", "share purchase agreement", "binding term sheet",
                    "definitive agreement", "letter of intent",
                ]
                if not any(kw in text for kw in strong):
                    continue
            else:
                continue
        ...
        if cat == "acqmerger" and has_demerger_word:
            ...
            if not any(kw in text for kw in non_merger):
                continue

        if cat == "results":
            other_kw = [kw for kw in rule["keywords"] if kw != "board meeting"]
            if "board meeting" in text and not any(kw in text for kw in other_kw):
...
def detect_acq_subcategory(text):
    t = text.lower()
...
def build_nse_link(ann):
    att = ann.get("attchmntFile", "")
    sym = ann.get("symbol", "")
    if att:
        return att if att.startswith("http") else f"https://www.nseindia.com{att}"
    if sym:
        return f"https://www.nseindia.com/get-quotes/equity?symbol={sym}#corporate-announcements"
    return "https://www.nseindia.com/companies-listing/corporate-filings-announcements"

def build_screener_link(ann):
    sym = ann.get("symbol", "")
    return f"https://www.screener.in/company/{sym}/announcements/" if sym else "https://www.screener.in"

# ══════════════════════════════════════════════════════════
# MESSAGE FORMATTERS
# ══════════════════════════════════════════════════════════

def format_message(ann, matched_cats, topic, category_label, nse_link, screener_link,
                   investor_name, is_first):
    ...
    if topic and topic != title:
        msg += f"📝 {topic}\n"
    msg += f"📅 {date_str}\n🔗 [NSE Circular]({nse_link})\n📈 [Screener]({screener_link})"
    return msg

def format_others_message(ann, nse_link, screener_link):
    company  = ann.get("sm_name") or ann.get("symbol", "Unknown")
    symbol   = ann.get("symbol", "")
    title    = ann.get("desc", "")
    date_str = ann.get("an_dt", "")
...
# ══════════════════════════════════════════════════════════
# TELEGRAM — proper retry_after handling, no time wasted
# ══════════════════════════════════════════════════════════

def send_to_channel(channel_id, msg, max_retries=6):
    if not channel_id:
        return
        ...
        try:
            ...
            if r.status_code == 429:
                try:
                    ...
                    retry_after = 30
                print(f"    [TG] 429 rate-limit — sleeping {retry_after}s "
            ...
            print(f"    [TG] Timeout attempt {attempt}/{max_retries}")
            if attempt < max_retries:
                time.sleep(10)
            ...
            if attempt < max_retries:
                time.sleep(5)
    print(f"    [TG] Failed after {max_retries} attempts — channel {channel_id}")

# ══════════════════════════════════════════════════════════
# GOOGLE SHEETS — row builder + bulk batch writer
# ══════════════════════════════════════════════════════════

def build_row(sheet_name, ann, category_label, topic, nse_link, screener_link,
              investor_name, is_first):
    company    = ann.get("sm_name") or ann.get("symbol", "Unknown")
    symbol     = ann.get("symbol", "")
    title      = ann.get("desc", "")
    ...
    if sheet_name == SHEET_INVESTORS:
        return [now, company, symbol, category_label, title,
                full_topic, investor_name, date_str, first_flag, nse_link, screener_link]
    elif sheet_name == SHEET_OTHERS:
        return [now, company, symbol, title, full_topic, date_str, nse_link, screener_link]
    else:
        return [now, company, symbol, category_label, title,
                full_topic, date_str, first_flag, nse_link, screener_link]


def flush_all_sheets(ws_map, pending_rows):
    """
    Bulk-write every queued row to every sheet.
    Called ONCE, before any Telegram messages are sent.
    Uses append_rows() (batch API) not append_row() (per-row API).
    """
    total = 0
    for sheet_name, rows in pending_rows.items():
        if not rows:
            print(f"  [SHEET] '{sheet_name}': 0 rows — skipping")
            continue
        ws = ws_map.get(sheet_name)
        if not ws:
            print(f"  [SHEET] WARNING: '{sheet_name}' missing from ws_map — "
                  f"skipping {len(rows)} rows")
            continue

        print(f"  [SHEET] '{sheet_name}': writing {len(rows)} row(s)…")
        written = 0
            ...
            for attempt in range(1, 6):
                try:
                    ...
                    if code == 429 or code >= 500:
                        wait = 25 * attempt
                        print(f"    [SHEET] API {code} — retry in {wait}s "
                              f"(attempt {attempt}/5)")
                        time.sleep(wait)
                    else:
                    ...
                    print(f"    [SHEET] Error writing chunk: {e}")
                    if attempt < 5:
                        time.sleep(10 * attempt)

        mark = "✅" if written == len(rows) else f"⚠️  {written}/{len(rows)}"
        print(f"  [SHEET] {mark} '{sheet_name}'")
        total += written

    print(f"  [SHEET] Total written: {total} rows across all sheets")
...
def cleanup_old_rows(ws_map):
    cutoff = datetime.now() - timedelta(days=CLEANUP_DAYS)
    print(f"\n[CLEANUP] Removing rows older than {cutoff.strftime('%Y-%m-%d')}…")
    for sheet_name, ws in ws_map.items():
        ...
        for idx in reversed(to_delete):
            try:
                ...
                print(f"  [CLEANUP] Error row {idx}: {e}")
...
def fetch_nse():
    ...
    now       = datetime.now()
    from_date = (now - timedelta(hours=24)).strftime("%d-%m-%Y")
    ...
    print(f"  Unique after dedup + sort: {len(unique)}")
    return unique
...
def setup_sheets():
    ...
    try:
        ...
        print(f"  [SHEET] ERROR: Invalid JSON — {e}")
        return {}
    ...
    inv_header = ["Logged At", "Company", "Symbol", "Category",
                  ...
                  "NSE Date", "First Disclosure?", "NSE Circular Link", "Screener Link"]
    oth_header = ["Logged At", "Company", "Symbol", "Title",
                  "Full Subject / Topic", "NSE Date", "NSE Circular Link", "Screener Link"]
...
def main():
    ...
    # ── PHASE 1: Fetch ─────────────────────────────────────
    print("\n[PHASE 1] Fetching NSE announcements (last 24h)...")
    ...
    if ws_map:
        ...
        print("  [SHEET] No connection -- sheet writes skipped")

    if ws_map:
        cleanup_old_rows(ws_map)

    # ── PHASE 2: Classify + queue everything ───────────────
    # Nothing is written to sheets or Telegram here.
    # Everything goes into memory buffers.
    print("\n[PHASE 2] Classifying all announcements...")

    pending_rows     = defaultdict(list)   # sheet_name -> [rows]
    pending_messages = []                  # [(channel_id, msg, log_line)]
    ...
    for ann in announcements:
        ...
        if not matched:
            ...
            pending_rows[SHEET_OTHERS].append(
                build_row(SHEET_OTHERS, ann, "", None, nse_link, screener_link, "", False)
            )
            pending_messages.append((
                CHANNEL_OTHERS,
                format_others_message(ann, nse_link, screener_link),
                f"  [OTHERS] {ann.get('symbol','?')} -- {title[:55]}"
            ))
            processed += 1
            continue
        ...
        channels = list(dict.fromkeys(RULES[c]["channel"] for c in matched))
        sheets   = list(dict.fromkeys(RULES[c]["sheet"]   for c in matched))

        for ch in channels:
            pending_messages.append((ch, msg, None))
        for sh in sheets:
            pending_rows[sh].append(
                build_row(sh, ann, category_label, topic,
                          nse_link, screener_link, investor_name, is_first)
            )
        for c in matched:
            counts[c] += 1
        ...
        print(f"  [QUEUED]{cross_note}{first_note} [{category_label}] -- "
              f"{ann.get('symbol','?')}")
        processed += 1

    print(f"\n  Classified: {processed} to write/send, {counts['excluded']} excluded")
    rows_summary = {k: len(v) for k, v in pending_rows.items() if v}
    print(f"  Sheet rows queued: {rows_summary}")
    print(f"  Telegram messages queued: {len(pending_messages)}")

    # ── PHASE 3: Write ALL 6 sheets FIRST ─────────────────
    # Data is 100% safe before any Telegram call is made.
    # GitHub Actions timeout during Phase 4 cannot lose sheet data.
    print("\n[PHASE 3] Writing all sheets (BEFORE sending Telegram)...")
    if ws_map:
        flush_all_sheets(ws_map, pending_rows)
    else:
        print("  [SHEET] Skipped -- no connection")

    # Save seen IDs right after sheets are confirmed written
    seen |= new_seen
    save_seen(seen)
    print(f"  Saved {len(new_seen)} new seen IDs to seen_ids.json")

    # ── PHASE 4: Send all Telegram messages ────────────────
    # If this phase times out, sheets are already safe.
    print(f"\n[PHASE 4] Sending {len(pending_messages)} Telegram message(s)...")
    tg_sent = 0
    for i, (channel_id, msg, log_line) in enumerate(pending_messages, 1):
        if log_line:
            print(log_line)
    ...
    print(f"  Telegram done: {tg_sent} message(s) sent")
    ...
    for cat, rule in RULES.items():
        print(f"  {rule['label']:<30}: {counts[cat]}")
    print(f"  New IDs saved   : {len(new_seen)}")
    for sn, rows in pending_rows.items():
        print(f"  Rows written -> '{sn}': {len(rows)}")
    print("=" * 55)
